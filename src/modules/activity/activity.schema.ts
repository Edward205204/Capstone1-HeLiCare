...
export const updateActivitySchema = [
  ...
  body('type').optional().isIn(Object.values(ActivityType)).withMessage('Invalid activity type'),
...
export const getActivitiesSchema = [
  ...
  query('type').optional().isIn(Object.values(ActivityType)).withMessage('Invalid activity type'),

  query('is_active').optional().isBoolean().withMessage('is_active must be a boolean'),

  query('search').optional().isLength({ max: 100 }).withMessage('Search term must not exceed 100 characters')
]
